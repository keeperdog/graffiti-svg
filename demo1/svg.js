...
var svg_db = {
  ...
  recovery() {
    ...
    if (this.recoveryStrokes.length) {
      ...
      if (stroke) {
        // 获取元素
        var strokeEl = this.strokeEls.find(
          (item) => item.getAttribute("id") === recoveryStroke.id
        );
        // 移除恢复列表
  ...
  clearComponents() {
    // 回调编辑对象
    if (this.selectStroke && this.editStroke) {
      this.selectStroke();
    }
    // 文本框失去焦点
    this.inputBlur();
  ...
  mouseUpEventClear(isReset, isCallback = true) {
    ...
    // 回调编辑对象
    if (this.selectStroke && isCallback) {
      this.selectStroke();
    }
  },
  // 注册鼠标监听
  ...
  mouseDownEventNew(e) {
    ...
    if (this.option.strokeType === "text") {
      ...
      // 回调编辑对象
      if (this.selectStroke) {
        this.selectStroke(this.inputStroke);
      }
      // 设置状态
      ...
      this.mouseMoveEvent = (oe) => {
        ...
        // 有移动范围则生效
        if (
          x > this.option.effectiveOffset.x ||
          y > this.option.effectiveOffset.y
        ) {
          // 回调
          if (this.mousemove) {
            this.mousemove(e);
  ...
  mouseDownEventMove(e) {
    ...
    if (isSafari && !strokeID.includes("text")) {
      this.svgEl.style.pointerEvents = "none";
    }
    // 查找编辑笔画对象
    if (!this.editStroke || this.editStroke.id !== strokeID) {
      ...
      // 回调编辑对象
      if (this.selectStroke) {
        this.selectStroke(this.editStroke, this.strokeEl);
      }
    }
    // 当前编辑笔画
  ...
  handleMouseUpEvent(e) {
    ...
    if (this.isNewStroke) {
      ...
      // 清空当前笔画元素
      if (this.stroke) {
        this.mouseUpEventClear(true, false);
      }
    }
    // Safari 浏览器处理
    if (isSafari) {
  ...
  drawChange(stroke, strokeEl) {
    ...
    if (stroke.type === "rect") {
      ...
      texts.forEach((item) => {
        // 回车使用全角空格替换，保证格式不变
        innerHTML += `<tspan x="${minx}" dy="${fontSize + fontLineSpace}">${
          item || "　"
        }</tspan>`;
      });
      ...
      strokeEl.setAttribute(
        "fill",
        this.option.isFill ? stroke.stroke : stroke.fill
      );
      ...
      strokeEl.setAttribute(
        "d",
        `M ${vertexs[0]} ${vertexs[1]} L ${vertexs[2]} ${vertexs[3]} L ${vertexs[4]} ${vertexs[5]} L ${vertexs[6]} ${vertexs[7]} L ${vertexs[8]} ${vertexs[9]} L ${vertexs[10]} ${vertexs[11]} Z`
      );
    }
  },
  // 创建编辑图形
  drawEditCreate() {
    ...
    // 显示
    var els = [
      this.editEls.circle1,
      this.editEls.circle2,
      this.editEls.circle3,
      this.editEls.circle4,
      this.editEls.circle5,
      this.editEls.circle6,
      this.editEls.circle7,
      this.editEls.circle8,
      this.editEls.rect,
    ];
    // 便利
    els.forEach((el) => {
  ...
  drawEditChange(stroke) {
    ...
    if (this.editEls.rect) {
      ...
      if (stroke.type === "text") {
        ...
        this.editEls.rect.setAttribute(
          "stroke-width",
          this.option.inputBoderWidth
        );
  ...
  inputCreate(e) {
    if (!this.inputEl) {
      // 创建笔画对象
      if (!this.inputStroke) {
        this.inputStroke = this.drawStrokeCreate(e);
      }
      // 引用
      var stroke = this.inputStroke;
      var svgSize = this.svgSize();
  ...
  inputStyleChange(stroke) {
    ...
    this.inputEl.style.lineHeight = `${
      stroke.fontSize + this.option.fontLineSpace
    }px`;
    // 复原输入框文本
    var texts = (stroke.text || "").split("\n");
    var innerHTML = "";
  ...
  inputScaleChange() {
    if (this.inputEl && this.inputStroke) {
      ...
      var scale = this.scaleValue(svgSize);
      var space =
        (this.option.inputPadding + this.option.inputBoderWidth) * scale;
      // 开始/结束坐标换算
  ...
  scaleValue(svgSize) {
    ...
    return this.option.maxHeight
      ? this.toFixed(svgSize.height / this.option.maxHeight)
      : 1;
  },
  // 自定义文本框全选内容
  selectText(el) {
