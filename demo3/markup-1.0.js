...
//适配移动端
CLOUD.DomUtil.adapterMobile = function (event, callback) {

    var touchFirst = event.touches[0];
    for (var key in touchFirst) {
        event[key] = touchFirst[key];
    }
    event.button = 0;
...
CLOUD.Extensions.Utils.Shape2D = {
    ...
    getRGBAString: function (hexRGBString, opacity) {
        ...
        parseInt('0x' + hexRGBString.substr(5, 2)), ',', opacity, ')'
        ].join('');
...
CLOUD.MiniMap = function (viewer) {
    ...
    this.addDomEventListeners = function () {
        ...
        if (_mapContainer) {
            ...
            _mapContainer.addEventListener('touchstart', function () {
                CLOUD.DomUtil.adapterMobile(event, that.onMouseDownBinded);
            }, false);
            _mapContainer.addEventListener('touchmove', function () {
                CLOUD.DomUtil.adapterMobile(event, that.onMouseMoveBinded);
            }, false);
            _mapContainer.addEventListener('touchend', function () {
    ...
    this.removeDomEventListeners = function () {
        ...
        if (_mapContainer) {
            ...
            _mapContainer.removeEventListener('touchstart', function () {
                CLOUD.DomUtil.adapterMobile(event, that.onMouseDownBinded);
            }, false);
            _mapContainer.removeEventListener('touchmove', function () {
                CLOUD.DomUtil.adapterMobile(event, that.onMouseMoveBinded);
            }, false);
            _mapContainer.removeEventListener('touchend', function () {
                CLOUD.DomUtil.adapterMobile(event, that.onMouseUpBinded);
            }, false);
...
CLOUD.Extensions.Marker.prototype = {
    ...
    addDomEventListeners: function () {
        ...
        var that = this;
        this.shape.addEventListener('touchstart', function (event) {
            CLOUD.DomUtil.adapterMobile(event, that.onMouseDownBinded);
        }, true);
    ...
    removeDomEventListeners: function () {
        ...
        this.shape.removeEventListener('touchstart', function (event) {
            CLOUD.DomUtil.adapterMobile(event, that.onMouseDownBinded);
        }, true);
...
// 设置屏幕基点
CLOUD.Extensions.Annotation2DEditor.prototype.setScreenBasePoint = function (point) {
...
CLOUD.Extensions.AnnotationHelper.prototype = {
    ...
    // 截屏 base64格式png图片
    captureAnnotationsScreenSnapshot: function (backgroundImage, callback) {

        var dataUrl = this.viewer.getRenderBufferScreenShot();
        dataUrl = this.editor.getScreenSnapshot(dataUrl, callback);
...
(function ($) {
    ...
    markup.prototype = {
        init: function () {
            ...
            var annotation = self.annotation = new CLOUD.Extensions.FreeAnnotationHelper();
        ...
        save: function (backgroundImage, callback) {
            ...
            var img = self.annotation.captureAnnotationsScreenSnapshot(backgroundImage, callback);
        ...
        controll: function () {
            ...
            },
            {
                ...
                children: {
                    type: 'color',
                    icon: 'bim-ncolor',
                    arr: ['#d0021b', '#ed4dad', '#2b84ed', '#7ed321', '#f8e71c', '#f58a23', '#5c5c5c', '#000000', '#ffffff']
                }
            },
            {
                id: 'line',
            ...
            function renderBar(obj) {
                switch (obj.type) {
                    case 'divede':
                        ...
                        break;
                    case 'more':
                        return '<div class="bar-item" data-type="' + obj.type + '"><i class="' + obj.icon + '"></i>' + renderBar(obj.children) + '</div>'
                        break;
                    case 'setMarkType':
                        var isSelected = obj.default ? "selected" : "";
                        return '<i class="bar-item ' + obj.icon + ' ' + isSelected + '" data-fn="' + obj.markType + '" data-type="' + obj.type + '"></i>';
                        break;
                    case 'text':
                        return '<div class="bar-item ' + obj.icon + '" data-fn="' + obj.markType + '" data-type="' + obj.type + '">' + renderBar(obj.children) + '</div>'
                        break;
                    case 'select':
                        var template = '<div class="subBar model-select"><select>';
                        $.each(obj.arr, function (index, item) {
                            template += '<option value="' + item + '">' + item + 'px</option>';
                        })
                        template += '<select></div>';
                        return template;
                        break;
                    case 'color':
                        ...
                        $.each(obj.arr, function (index, item) {
                            template += '<span class="bar-item bar-color" data-color="' + item + '"><i class="' + obj.icon + '" style="background:' + item + ';"></i></span>';
                        })
                        template += '<div>';
                        return template;
                        break;
                    case 'line':
                        var template = '<div class="subBar">';
                        $.each(obj.arr, function (index, item) {
                            template += '<span class="bar-item bar-line" data-height="' + item + '"><i class="' + obj.icon + '" style="height:' + item + 'px;margin:' + (17 - item / 2) + 'px auto;"></i></span>';
                        })
                        template += '<div>';
                        return template;
                        break;
            ...
            toolBar.on('click', '.bar-item', function () {
                var $this = $(this),
                    type = $this.data('type'),
                    fn = $this.data('fn');
                switch (type) {
                ...
                self.annotation.setAnnotationStyle($.extend(defaultStyle, {
                    "stroke-color": color,
                }), true)
...
(function ($) {
    ...
    markPdf.prototype = {
        ...
        begin: function () {
            var self = this;
            self.annotation.editAnnotationBegin(self._option.pointToCenter, self._option.screenBasePoint, {
        ...
        load: function (annotations, absBasePoint, screenBasePoint, zoomFactor) {
            var self = this;
            self.annotation.loadAnnotations(annotations, absBasePoint, screenBasePoint, {
                x: zoomFactor,
                y: zoomFactor
            })
        },
